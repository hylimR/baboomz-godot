        ...
        static void UpdateTerritories(GameState state, float dt)
        {
            ...
            for (int t = 0; t < teamCount; t++)
            {
                if (territory.TeamScores[t] >= state.Config.TerritoryPointsToWin)
                {
                    ...
                    // Find first alive player on winning team; fall back to
                    // first player on team if all are dead.
                    int fallback = -1;
                    for (int i = 0; i < state.Players.Length; i++)
                    {
                        ...
                        {
                            state.WinnerIndex = i;
                            fallback = -1; // signal that we found an alive player
                            break;
                        }
                    }
                    if (fallback >= 0)
                        state.WinnerIndex = fallback;
                    return;
                }
            }
        }
